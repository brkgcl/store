import React from 'react';

const appData = [
  {
    name: 'game',
    img: 'bg-[url("https://cdn.dribbble.com/users/465131/screenshots/14718799/media/ca9b2df64b8b4510ec4957a363038f6b.jpg?compress=1&resize=400x300")]',
    url: '/game',
    color: 'bg-red-400',
  },
  {
    name: 'music',
    img: 'bg-[url("data:image/jpeg;base64,")]',
    url: '/music',
    color: 'bg-indigo-800',
  },
  {
    name: 'social',
    img: 'bg-[url("https://static8.depositphotos.com/1005738/908/v/600/depositphotos_9080572-stock-illustration-social-media-talk-bubble-texture.jpg")]',
    url: '/social',
    color: 'bg-yellow-400',
  },
  {
    name: 'sports',
    img: 'bg-[url("https://getwallpapers.com/wallpaper/full/d/3/8/371378.jpg")]',
    url: '/sports',
    color: 'bg-blue-500',
  },
  {
    name: 'tools',
    img: 'bg-[url("https://img.freepik.com/free-photo/top-view-many-tools_23-2147773434.jpg")]',
    url: '/tools',
    color: 'bg-orange-600',
  },
  {
    name: 'development',
    img: 'bg-[url("https://media.istockphoto.com/id/871030872/photo/programming-code-abstract-technology-background-of-software-developer.jpg?s=170667a&w=0&k=20&c=4Hxpl5eAm5YAKyuA3qMVEJ62TMEV4dYasWBZ_RwcyKQ=")]',
    url: '/development',
    color: 'bg-black',
  },
];

const newData = [
  {
    title: 'NinjaGame',
    info: 'Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet.',
    img: 'https://cdn.dribbble.com/users/465131/screenshots/14718799/media/ca9b2df64b8b4510ec4957a363038f6b.jpg?compress=1&resize=400x300',
  },
  {
    title: 'Play Piano',
    info: 'Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet.',
    img: 'data:image/jpeg;base64,',
  },
  {
    title: 'Me2Talk',
    info: 'Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet.',
    img: 'https://static8.depositphotos.com/1005738/908/v/600/depositphotos_9080572-stock-illustration-social-media-talk-bubble-texture.jpg',
  },
  {
    title: 'Score-News',
    info: 'Lorem ipsum dolor sit amet. Lorem ipsum dolor ',
    img: 'https://getwallpapers.com/wallpaper/full/d/3/8/371378.jpg',
  },
  {
    title: 'Physics Adventure',
    info: 'Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet.',
    img: 'https://img.freepik.com/free-photo/top-view-many-tools_23-2147773434.jpg',
  },
  {
    title: 'DevConsol',
    info: 'Lorem ipsum dolor sit amet. ',
    img: 'https://media.istockphoto.com/id/871030872/photo/programming-code-abstract-technology-background-of-software-developer.jpg?s=170667a&w=0&k=20&c=4Hxpl5eAm5YAKyuA3qMVEJ62TMEV4dYasWBZ_RwcyKQ=',
  },
  //  {title: '', info: '', img: ''},
  //  {title: '', info: '', img: ''},
  //  {title: '', info: '', img: ''},
  //  {title: '', info: '', img: ''},
  //  {title: '', info: '', img: ''},
  //  {title: '', info: '', img: ''},
  //  {title: '', info: '', img: ''},
  //  {title: '', info: '', img: ''},
];
const TopicData = [
  {
    title: 'NinjaGame',
    img: 'https://cdn.dribbble.com/users/465131/screenshots/14718799/media/ca9b2df64b8b4510ec4957a363038f6b.jpg?compress=1&resize=400x300',
  },
  {
    title: 'Play Piano',
    img: 'data:image/jpeg;base64,',
  },
  {
    title: 'Me2Talk',
    img: 'https://static8.depositphotos.com/1005738/908/v/600/depositphotos_9080572-stock-illustration-social-media-talk-bubble-texture.jpg',
  },
  {
    title: 'Score-News',
    img: 'https://getwallpapers.com/wallpaper/full/d/3/8/371378.jpg',
  },
  {
    title: 'Physics Adventure',
    img: 'https://img.freepik.com/free-photo/top-view-many-tools_23-2147773434.jpg',
  },
  {
    title: 'DevConsol',
    img: 'https://media.istockphoto.com/id/871030872/photo/programming-code-abstract-technology-background-of-software-developer.jpg?s=170667a&w=0&k=20&c=4Hxpl5eAm5YAKyuA3qMVEJ62TMEV4dYasWBZ_RwcyKQ=',
  },
  {
    title: 'NinjaGame',
    img: 'https://cdn.dribbble.com/users/465131/screenshots/14718799/media/ca9b2df64b8b4510ec4957a363038f6b.jpg?compress=1&resize=400x300',
  },
  {
    title: 'Play Piano',
    img: 'data:image/jpeg;base64,',
  },
  {
    title: 'Me2Talk',
    img: 'https://static8.depositphotos.com/1005738/908/v/600/depositphotos_9080572-stock-illustration-social-media-talk-bubble-texture.jpg',
  },
  {
    title: 'Score-News',
    img: 'https://getwallpapers.com/wallpaper/full/d/3/8/371378.jpg',
  },
  {
    title: 'Physics Adventure',
    img: 'https://img.freepik.com/free-photo/top-view-many-tools_23-2147773434.jpg',
  },
  {
    title: 'DevConsol',
    img: 'https://media.istockphoto.com/id/871030872/photo/programming-code-abstract-technology-background-of-software-developer.jpg?s=170667a&w=0&k=20&c=4Hxpl5eAm5YAKyuA3qMVEJ62TMEV4dYasWBZ_RwcyKQ=',
  },
];

const Home = () => {
  return (
    <div className="h-full bg-primary  text-white px-8 pb-2 overflow-y-scroll">
      <h2 className='text-white text-2xl p-3 pt-8 font-bold"'>News</h2>
      <div className="h-1/3 ">
        <div className="h-full w-full flex gap-2">
          <div className="flex-[2] flex h-full bg-[#272b39] rounded-2xl p-2 drop-shadow-lg">
            <img className="h-full" src="work.png" alt="" />
            <div className="flex flex-col justify-center gap-1 p-2">
              <h2 className="text-xl font-bold">Lorem ipsum dolor sit amet.</h2>
              <p className="text-xs">
                Lorem ipsum dolor, sit amet consectetur adipisicing elit.
                Perspiciatis quisquam expedita iusto reiciendis sunt?Lorem ipsum
                dolor sit amet Lorem ipsum dolor sit amet consectetur
                adipisicing elit.
              </p>
              <div className="p-3 w-full flex justify-end">
                <button className="bg-indigo-800 w-1/3 rounded-2xl p-2 flex items-center justify-center">
                  continue
                </button>
              </div>
            </div>
          </div>
          <div className="flex-1 flex flex-col h-full gap-2">
            <div className=" bg-[#272b39] h-1/2 drop-shadow-2xl rounded-2xl flex justify-between p-3">
              <div className="flex flex-col justify-center">
                <h2>Lorem ipsum dolor sit amet</h2>
                <p className="text-xs">
                  Lorem ipsum dolor sit amet consectetur adipisicing elit.
                  Itaque, delectus?
                </p>
              </div>
              <img className="h-4/5 " src="component_icon.png" alt="" />
            </div>
            <div className=" bg-[#272b39] h-1/2 drop-shadow-2xl rounded-2xl flex justify-between p-3 gap-2">
              <img src="mobile.png" className="h-4/5" alt="" />
              <div>
                <p className="text-sm">
                  Lorem ipsum dolor sit amet consectetur adipisicing elit.
                  Aperiam delectus, consectetur fugiat in
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <h2 className="text-white text-2xl p-3 pt-8 font-bold">Most Popular</h2>
      <div className="flex  items-center gap-4 overflow-x-scroll">
        {appData.map((e) => (
          <div
            className={`w-1/3 h-36 ${e.img} after:opacity-40 rounded-lg cursor-pointer hover:opacity-20 shadow-2xl`}
          >
            {/* <img
              className="absolute w-44 h-36 rounded-lg opacity-40 shadow-2xl"
              src={e.img}
              alt=""
            /> */}
            <div className=" flex flex-col items-stretch gap-16 px-4 py-1 ">
              <h2 className="text-xl font-bold">{e.name}</h2>
              <div className="flex  items-end justify-end ">
                {/* <p>Lorem</p> */}
                <button className="bg-white text-indigo-700 rounded-lg  px-4 hover:bg-indigo-700 hover:text-white">
                  open
                </button>
              </div>
            </div>
          </div>
        ))}
      </div>
      <div className="flex w-full pb-20">
        <div className="w-1/2">
          <h2 className="text-white text-2xl p-3 pt-8 pb-3 font-bold">New</h2>
          <div className="flex flex-col h-80 w-full shadow-lg overflow-y-auto pr-3">
            {newData.map((e) => (
              <div className="flex w-full justify-between hover:bg-gray-700 cursor-pointer p-2 rounded-md">
                <div className="h-16 w-full   rounded-lg flex  gap-6 items-center  ">
                  <img className="h-14 w-14 rounded-lg " src={e.img} alt="" />
                  <div className="flex w-full items-center justify-between gap-2">
                    <div className="flex flex-col">
                      <h2 className="text-xl font-bold">{e.title}</h2>
                      <p className="text-xs">{e.info.substring(0, 50)}..</p>
                    </div>
                    <div className="bg-indigo-400 p-2 rounded-lg cursor-pointer hover:bg-indigo-800">
                      download
                    </div>
                  </div>
                </div>
              </div>
            ))}
          </div>
        </div>
        <div className="w-1/2 pb-20">
          <h2 className="text-white text-2xl p-3 pt-8 font-bold">Topic</h2>
          <div className="grid gap-1 grid-cols-4 grid-rows-3 w-full h-full ">
            {TopicData.map((e) => (
              <div className=" bg-nav2 rounded-lg hover:scale-105 flex items-center justify-center cursor-pointer">
                <div className="flex flex-col items-center justify-center gap-1">
                  <img className="h-12 w-12 rounded-full" src={e.img} alt="" />
                  <h1 className="text-sm">{e.title}</h1>
                </div>
              </div>
            ))}
          </div>
        </div>
      </div>
    </div>
  );
};

export default Home;

/**
 * <div className="w-1/3 h-36  rounded-lg cursor-pointer ">
          <img
            className="absolute w-44 h-36 rounded-lg opacity-20 "
            src="https://cdn.dribbble.com/users/465131/screenshots/14718799/media/ca9b2df64b8b4510ec4957a363038f6b.jpg?compress=1&resize=400x300"
            alt=""
          />
          <div className="relative flex flex-col items-stretch gap-16 px-4 py-1 ">
            <h2 className="text-2xl font-bold">Game</h2>
            <div className="flex justify-between items-center">
              <p>Lorem</p>
              <button className="bg-indigo-700 rounded-lg  px-4">open</button>
            </div>
          </div>
        </div>
 */

/**
         * <div className="flex w-full justify-between ">
              <div className="h-16 w-full   rounded-lg flex  gap-6 items-center ">
                <img
                  className="h-14 w-14 rounded-lg "
                  src="https://getwallpapers.com/wallpaper/full/d/3/8/371378.jpg"
                  alt=""
                />
                <div className="flex flex-col">
                  <h2 className="text-xl font-bold">Physics Adventure</h2>
                  <p className="text-xs">
                    Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet.
                  </p>
                </div>
                <div className="bg-indigo-400 p-2 rounded-lg cursor-pointer">
                  download
                </div>
              </div>
            </div>
         */
